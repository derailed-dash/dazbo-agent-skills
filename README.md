...
| **Deploying Skills** | Automated installer, relocator, and manager for Antigravity/Gemini agent skills. Saves time by automatically managing local and global paths. | **Use when** downloading, importing, or updating new agent skills. Automates the installation process via `npx` and safely moves skills to the shared global path (`~/.gemini/config/skills/`). |
